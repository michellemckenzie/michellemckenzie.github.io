...
(function(){
    ...
    var gameData = {
        ...
        answer: 0,
        strike: [0,0],
        strikescreen: ['p1strike', 'p2strike'],
    };
    ...
    startGame.addEventListener('click', function(){
        ...
        document.getElementById(`${gameData.cards[gameData.index]}`).style.backgroundColor = 'rgba(191, 255, 37, 0.11)';

        document.getElementById(`${gameData.cards[gameData.index]}`).style.boxShadow = '0px 6px 10px rgb(167, 164, 164)';
    ...
    function randomCircle(){
        ...
        circle[number_of_circles].style.background = "linear-gradient(0deg, rgba(152,231,251,1) 0%, rgba(253,255,188,1) 100%)";

        circle[number_of_circles].style.boxShadow = "";
    ...
    function dropBubbles(){
        ...
        if (gameData.index == 0){
            ...
            // number_of_circles = gameData.p2bubbletop.length;
        }
        else{
            gameData.p2bubbles = [];        
            gameData.p2bubbletop = [];

            // number_of_circles = gameData.p1bubbletop.length;
        }
    ...
    function submitButton(e){
        ...
        if (answer == gameData.answer){
            ...
            gameData.strike[gameData.index] += 1;

            document.getElementById(`${gameData.strikescreen[gameData.index]}`).textContent = `strike: ${gameData.strike[gameData.index]}`;

            if (gameData.strike[gameData.index] == 3){
                dropBubbles();
                gameData.strike[gameData.index] = 0;
                gameData.score[gameData.index] = 0;
                document.getElementById(`${gameData.ppoints[gameData.index]}`).textContent = `${gameData.score[gameData.index]} pts`;

                setTimeout(document.getElementById(`${gameData.strikescreen[gameData.index]}`).textContent = `strike: ${gameData.strike[gameData.index]}`, 3000);
            }

            setTimeout(changePlayer(), 6000);
        }
    }

    function changePlayer(){
        document.getElementById(`${gameData.cards[gameData.index]}`).style.backgroundColor = '';

        document.getElementById(`${gameData.cards[gameData.index]}`).style.boxShadow = '';
